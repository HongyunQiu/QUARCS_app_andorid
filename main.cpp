...
int main(int argc, char *argv[])
{
    ...
    // 注册 WifiInfoProvider 类
    qmlRegisterType<WifiInfoProvider>("CustomTypes", 1, 0, "WifiInfoProvider");
    // 注册 BluetoothScanner 类（保留原有实现）
    qmlRegisterType<BluetoothScanner>("CustomTypes", 1, 0, "BluetoothScanner");
    // 注册 QtBluetoothScanner 类（基于 Qt Bluetooth 模块的新实现）
    qmlRegisterType<QtBluetoothScanner>("CustomTypes", 1, 0, "QtBluetoothScanner");
    ...
    AppHostService appHostService(appversion, &wifiInfoProvider);
    EmbeddedAssetHttpServer embeddedAssetHttpServer(&appHostService);
    const bool embeddedFrontendEnabled = embeddedAssetHttpServer.start(18080);
    CompatibilityCommandServer compatibilityCommandServer(&appHostService);
    const bool compatibilityBridgeEnabled = compatibilityCommandServer.start(8600);

    const QUrl embeddedFrontendUrl = embeddedAssetHttpServer.baseUrl();

    engine.rootContext()->setContextProperty("appName", appName);
    engine.rootContext()->setContextProperty("appVersion", appversion);
    engine.rootContext()->setContextProperty("appHostService", &appHostService);
    engine.rootContext()->setContextProperty("embeddedFrontendEnabled", embeddedFrontendEnabled && compatibilityBridgeEnabled);
    engine.rootContext()->setContextProperty("embeddedFrontendUrl", embeddedFrontendUrl);
    engine.rootContext()->setContextProperty("embeddedFrontendAutoLaunch", embeddedFrontendEnabled && compatibilityBridgeEnabled);
    engine.rootContext()->setContextProperty("compatibilityCommandServer", &compatibilityCommandServer);
